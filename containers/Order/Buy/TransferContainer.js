...
const TransferContainer = (props) => {
  const { orderData, handleBuyerConfirm, period, handleCancelOrder, navigation } =  props;
  ...
  return (
    <>
      ...
      <View style={[tw`flex flex-row items-center mb-4 rounded-tr-lg rounded-tl-lg`, {backgroundColor: "#060e0c"}]}>
        <View style={[tw`flex-1 border-b-2 py-2 flex h-24 justify-center`, {borderColor: "#68CEC7"}]}>
      ...
      <View style={tw`mb-4`}>
        ...
        <TextComponent color="gray" size="sm">Once payment is completed, click on "payment done"</TextComponent>
        <TextComponent color="gray" size="sm">Risk disclaimer: once timer runs out and you have not send the payment or clicked ‘payment done’, seller will be able to withdraw asset from the smart contract.</TextComponent>
      </View>
      <View style={tw`flex flex-row items-center mb-8 justify-between`}>
        <GrayButton title="Cancel" onPress={handleCancelOrder} />
        <GreenButton size="lg" title="Payment Done" active onPress={() => setShowModal(true)} />
      </View>
      <BuyerPaymentModal
        isShown={showModal}
